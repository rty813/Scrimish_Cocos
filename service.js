...
for (let i = 0; i < MAX; i++) {
    rooms[i].socket = io.of('/rooms' + i).on('connection', function(socket) {
        ...
        socket.on('disconnect', function() {
            ...
            socket.broadcast.emit('someone disconnect');
            console.log('someone disconnected room' + i + '.There are ' + rooms[i].people + ' people in the room');
        });

        socket.on('ready', function() {
            rooms[i].ready++;
            console.log(rooms[i].ready + 'ready!');
            if (rooms[i].ready === 2) {
                console.log('game start');
                rooms[i].socket.emit('game start', null);
            }
        });

        socket.on('attack', function(args) {
            socket.broadcast.emit('be attacked', args);

        });

        socket.on('response', function(card) {
            socket.broadcast.emit('attack result', card);
        });

        socket.on('discard', function(pile) {
            socket.broadcast.emit('discard', pile);
